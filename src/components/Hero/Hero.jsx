...
const Hero = () => {
  ...
  // Фон карточки в зависимости от темы
  ...
  return (
    <div id="home" className="overflow-hidden relative">
      ...
      {/* УВЕЛИЧИВАЕМ ОТСТУП ЕЩЕ БОЛЬШЕ ДЛЯ МОБИЛЬНЫХ */}
      <div
        className={`pt-32 md:pt-20 min-h-[700px] sm:min-h-[800px] ${heroBg} flex justify-center items-center relative z-10`}
      >
            ...
            {HeroData.map((data, index) => (
              <div key={data.id}>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center min-h-[500px] sm:min-h-[600px]">
                  {/* Text Content */}
                  <div className="flex flex-col justify-center gap-4 sm:gap-6 text-center lg:text-left order-2 lg:order-1">
                    {/* Animated Badge */}
                    {data.badge && (
                      <div
                        data-aos="fade-down"
                    ...
                    <h1
                      ...
                      data-aos-delay="100"
                      className="text-xl sm:text-2xl font-medium text-gray-500 dark:text-gray-400 tracking-widest uppercase"
                    >
                      {data.subtitle}
                    </h1>
                    ...
                    >
                      <Link
                        to="/products"
                        className={`group relative px-8 py-4 text-lg font-semibold text-white rounded-xl bg-gradient-to-r ${data.accentColor} transition-all duration-300 transform hover:scale-105 hover:shadow-2xl overflow-hidden`}
                      >
                        <span className="relative z-10">Купить сейчас</span>
                        <div className="absolute inset-0 bg-white/20 transform -skew-x-12 -translate-x-full group-hover:translate-x-full transition-transform duration-1000" />
                      </Link>

                      {/* УБИРАЕМ КНОПКУ "НАВЕДИ НА КАРТОЧКУ" НА МОБИЛЬНЫХ */}
                      <div className="hidden sm:block px-8 py-4 text-lg font-semibold border-2 border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-xl transition-all duration-300 hover:border-transparent hover:bg-white/10 hover:backdrop-blur-sm hover:shadow-lg cursor-default">
                      ...
                      <div className="hero-flip-card-inner">
                        ...
                        <div className="hero-flip-card-front">
                          ...
                          >
                            ...
                            {/* Price Tag - ПОДНИМАЕМ ЕЩЕ ВЫШЕ */}
                            <div
                              className={`absolute -top-1 -right-1 sm:-top-4 sm:-right-4 px-3 py-1 sm:px-4 sm:py-2 bg-gradient-to-r ${data.accentColor} text-white text-xs sm:text-sm font-bold rounded-full shadow-lg transform rotate-12 animate-float z-20`}
                            >
                              {data.price}
                            </div>

                            {/* Hover Hint - ТОЛЬКО ДЛЯ ДЕСКТОПА */}
                        ...
                        <div className="hero-flip-card-back">
                          ...
                          >
                            <div>
                              ...
                              <p className="text-sm sm:text-lg opacity-90 text-center mb-4 sm:mb-6 leading-relaxed">
